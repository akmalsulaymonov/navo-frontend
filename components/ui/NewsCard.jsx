...
export default function NewsCard({ article, size = 'md', horizontal }) {
  ...
  if (horizontal) {
    return (
      <div
        ...
        className="group flex gap-4 cursor-pointer pb-4 border-b border-gray-200 mb-4"
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[14px] font-semibold text-gray-900 group-hover:text-secondary leading-snug mt-1.5 transition-colors line-clamp-2">{displayTitle}</div>
  ...
  return (
    <div
      onClick={() => router.push(`/article/${article.slug}`)}
      className="group cursor-pointer bg-white rounded-md overflow-hidden shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-200 flex flex-col"
    >
      ...
      <div className="p-4 pb-5 flex-1 flex flex-col">
        ...
        <div
          className="font-semibold text-gray-900 group-hover:text-secondary mt-2.5 mb-2 transition-colors line-clamp-3"
          style={{ fontSize: size === 'lg' ? 20 : size === 'sm' ? 15 : 17, lineHeight: 1.4 }}
        >{displayTitle}</div>
        {size !== 'sm' && (
          <div className="text-[14px] text-gray-500 leading-relaxed flex-1 line-clamp-2">{displayExcerpt}</div>
        )}
        <div className="flex items-center justify-between mt-3.5 pt-3.5 border-t border-soft">
          <div className="flex items-center gap-1.5">
            <AuthorAvatar name={article.author.name} size={24} style={{ border: 'none' }} />
            <span className="text-[13px] font-medium text-gray-900">{article.author.name}</span>
          </div>
          <span className="text-[12px] text-gray-500">{article.date}</span>
        </div>
      </div>
