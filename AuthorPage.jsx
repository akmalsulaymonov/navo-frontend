// NAVO Author Page
const AuthorPage = ({ author, navigate }) => {
  ...
  // Pad to 10 so we always show 1 hero + 9 grid cards
  const allArticles = authorArticles.length >= 10
    ? authorArticles
    : [
        ...authorArticles,
        ...NAVO_DATA.articles.filter(a => !authorArticles.includes(a)).slice(0, 10 - authorArticles.length),
      ];
  ...
  return (
    <div style={{ background: C.bg }}>
      ...
      {/* ── AUTHOR HERO BANNER ── */}
      <div style={{
        background: `linear-gradient(135deg, #1a1f4e 0%, #323F90 60%, #1486C8 100%)`,
        ...
        <div style={{
          maxWidth: 1280, margin: '0 auto', padding: '56px 24px 0',
          display: 'flex', gap: 48, alignItems: 'flex-end', position: 'relative',
        }}>
          {/* Avatar */}
          <div style={{
            flexShrink: 0,
            ...
            <div style={{
              ...
              <img
                src={`https://images.unsplash.com/${author.photoId}?auto=format&fit=crop&w=320&h=320&q=80`}
                alt={author.name}
          ...
          <div style={{
            ...
            opacity: headerVisible ? 1 : 0,
            transition: 'all 0.5s cubic-bezier(.22,1,.36,1) 0.1s',
          }}>
            {/* Role badge */}
            <div style={{
              ...
              background: 'rgba(255,255,255,0.12)', borderRadius: 3, padding: '4px 10px',
            }}>{author.role}</div>

            <h1 style={{
              fontFamily: 'Montserrat, sans-serif', fontSize: 42, fontWeight: 800,
              color: '#fff', margin: '0 0 8px', letterSpacing: '-0.8px', lineHeight: 1.1,
            }}>{author.name}</h1>

            <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 18, flexWrap: 'wrap' }}>
              <span style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 15, color: 'rgba(255,255,255,0.75)', fontWeight: 500 }}>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {author.social?.twitter && (
                <div style={{
                  ...
                  border: '1px solid rgba(255,255,255,0.25)', borderRadius: 4, padding: '5px 12px',
          ...
          {/* Stats — right side */}
          <div style={{
            display: 'flex', gap: 2, paddingBottom: 0, flexShrink: 0,
            transform: headerVisible ? 'translateY(0)' : 'translateY(20px)',
            opacity: headerVisible ? 1 : 0,
            ...
            {statItems.map((s, i) => (
              <div key={i} style={{
                ...
                borderRadius: i === 0 ? '8px 0 0 0' : i === statItems.length - 1 ? '0 8px 0 0' : 0,
                padding: '20px 28px', textAlign: 'center', minWidth: 110,
              }}>
          ...
          <div style={{ display: 'flex', gap: 2, marginTop: 32, borderTop: '1px solid rgba(255,255,255,0.12)', paddingTop: 2 }}>
            {cats.map(cat => (
              <button key={cat} onClick={() => setActiveFilter(cat)} style={{
                fontFamily: 'Montserrat, sans-serif', fontSize: 13, fontWeight: activeFilter === cat ? 700 : 500,
                background: activeFilter === cat ? 'rgba(255,255,255,0.15)' : 'transparent',
                border: 'none', borderBottom: activeFilter === cat ? '3px solid #fff' : '3px solid transparent',
                color: activeFilter === cat ? '#fff' : 'rgba(255,255,255,0.6)',
                padding: '12px 18px', cursor: 'pointer', transition: 'all 0.15s', borderRadius: '4px 4px 0 0',
              }}>{cat}</button>
            ))}
          </div>
        </div>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '40px 24px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 300px', gap: 48 }}>
          ...
          <div>
            {/* Bio card */}
            <div style={{
              background: C.bgSoft, borderRadius: 8, padding: '24px 28px',
              marginBottom: 40, borderLeft: `4px solid ${C.primary}`,
              ...
              <p style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 15, color: '#444', lineHeight: 1.75, margin: 0 }}>
                {author.bio}
              </p>
            </div>
            ...
            {heroArticle && (
              <div style={{ marginBottom: 40 }}>
                ...
                <div
                  onClick={() => navigate('article', heroArticle)}
                  style={{
                    display: 'grid', gridTemplateColumns: '1fr 1fr', borderRadius: 10,
                ...
                >
                  <div style={{ position: 'relative' }}>
                    <img
                      src={`https://images.unsplash.com/${getImgId(heroArticle.slug, heroArticle.category)}?auto=format&fit=crop&w=700&q=80`}
                  ...
                  <div style={{ padding: '32px 28px', background: '#fff', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                    ...
                    <h2 style={{
                      ...
                      color: C.textPrimary, lineHeight: 1.3, margin: '14px 0 12px', letterSpacing: '-0.3px',
                    }}>{heroArticle.title}</h2>
                    <p style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 14, color: C.textSecondary, lineHeight: 1.65, marginBottom: 20 }}>
                      {heroArticle.excerpt}
                    </p>
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
                  {gridArticles.map((a, i) => (
                    <React.Fragment key={a.slug || a.id}>
                      {i === 6 && (
                        <div style={{ gridColumn: '1 / -1', display: 'flex', justifyContent: 'center', margin: '8px 0' }}>
                          <AdBlock w={728} h={90} label="author-page-infeed" />
                        </div>
                      )}
                      <NewsCard article={a} navigate={navigate} />
                    </React.Fragment>
                  ))}
                </div>
                {filtered.length <= 1 && (
                  <div style={{ gridColumn: '1 / -1' }}>
                    <p style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 14, color: C.textSecondary, textAlign: 'center', padding: '40px 0' }}>
                      More articles by {author.name} coming soon.
                    </p>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 24 }}>
                      {NAVO_DATA.articles.filter(a => a.id !== heroArticle?.id).slice(0, 6).map(a => (
