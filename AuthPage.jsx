    ...
    <div style={{ minHeight: '70vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: C.bgSoft }}>
      <div style={{ textAlign: 'center', padding: 48 }}>
        ...
        <button onClick={() => navigate('home')} style={{
          ...
          borderRadius: 6, padding: '13px 32px', cursor: 'pointer',
    ...
    <div style={{ minHeight: '80vh', background: C.bgSoft, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '40px 24px' }}>
      <div style={{ width: '100%', maxWidth: 440 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          <span onClick={() => navigate('home')} style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 28, fontWeight: 800, color: C.primary, cursor: 'pointer', letterSpacing: '-0.5px' }}>NAVO</span>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', borderBottom: `1px solid ${C.neutral}` }}>
            {['login', 'register'].map(t => (
              <button key={t} onClick={() => setTab(t)} style={{
                fontFamily: 'Montserrat, sans-serif', fontSize: 14, fontWeight: tab === t ? 700 : 500,
                background: tab === t ? '#fff' : C.bgSoft,
                color: tab === t ? C.primary : C.textSecondary,
                border: 'none', borderBottom: tab === t ? `3px solid ${C.primary}` : '3px solid transparent',
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 24 }}>
              {socials.map(s => (
                <button key={s.name} style={{
                  ...
                  border: `1px solid ${C.neutral}`, borderRadius: 6, padding: '10px 8px', cursor: 'pointer',
            ...
            <form onSubmit={handleSubmit}>
              {tab === 'register' && (
                <div style={{ marginBottom: 16 }}>
                  ...
                  <input
                    ...
                    style={{ width: '100%', height: 44, fontFamily: 'Montserrat, sans-serif', fontSize: 14, border: `1px solid ${C.neutral}`, borderRadius: 6, padding: '0 14px', outline: 'none', boxSizing: 'border-box', color: C.textPrimary, transition: 'border-color 0.15s' }}
                    onFocus={e => e.target.style.borderColor = C.primary}
                    onBlur={e => e.target.style.borderColor = C.neutral}
                  />
                </div>
              )}
              ...
              <div style={{ marginBottom: tab === 'register' ? 8 : 24 }}>
                ...
                <div style={{ position: 'relative' }}>
                  <input
                    ...
                    style={{ width: '100%', height: 44, fontFamily: 'Montserrat, sans-serif', fontSize: 14, border: `1px solid ${C.neutral}`, borderRadius: 6, padding: '0 44px 0 14px', outline: 'none', boxSizing: 'border-box', color: C.textPrimary, transition: 'border-color 0.15s' }}
                    onFocus={e => e.target.style.borderColor = C.primary}
                    onBlur={e => e.target.style.borderColor = C.neutral}
                  />
                  <button type="button" onClick={() => setShowPass(s => !s)} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: C.textSecondary, fontSize: 14 }}>
                    {showPass ? '🙈' : '👁'}
                  </button>
                </div>
              </div>
              ...
              {tab === 'register' && password && (
                <div style={{ marginBottom: 20 }}>
                  ...
                  <span style={{ fontFamily: 'Montserrat, sans-serif', fontSize: 12, color: strengthColors[strength], fontWeight: 600 }}>
              ...
              <button type="submit" disabled={loading} style={{
                width: '100%', height: 48, fontFamily: 'Montserrat, sans-serif', fontSize: 15, fontWeight: 700,
                background: loading ? '#8a95c4' : C.primary, color: '#fff', border: 'none',
                borderRadius: 6, cursor: loading ? 'wait' : 'pointer', transition: 'background 0.2s',
              }}
              ...
              >
                {loading ? 'Please wait…' : tab === 'login' ? 'Log In to NAVO' : 'Create Account'}
              </button>
            </form>
